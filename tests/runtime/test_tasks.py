...
class TestPredictionTask:
    ...
    def test_configurable_loss_and_metrics(self) -> None:
        task = PredictionTask(loss_name='mae', metrics=['mse'])
        preds = np.ones((4, 12, 3))
        targets = np.zeros((4, 12, 3))
        loss = task.build_loss({'predictions': preds}, {'y': targets})
        metrics = task.compute_metrics({'predictions': preds}, {'y': targets})
        assert loss == pytest.approx(1.0)
    ...
    def test_default_metrics_list(self) -> None:
        task = PredictionTask()
        assert set(task.default_metrics) == {'rmse', 'mae', 'nse'}
