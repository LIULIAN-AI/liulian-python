...
### Part 4 — Deep Audit & Missing-Model Extension (IN PROGRESS)

- Added explicit **no-script argument provenance** appendix for all 40 `Script=❌` pairs
- Added per-pair **non-match diagnosis** appendix for 26 non-matching pairs (excluding PatchTST/DLinear)
- Clarified missing-model status matrix for TimeLLM / GPT4TS / TimeMoE / ETSformer / Stationary
- Pending (after latest reruns finish): merge updated `tsl_comparison_results.json` and refresh final verdicts

---

## Part A — Feature Inventorya
