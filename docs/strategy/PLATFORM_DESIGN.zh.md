...
LIULIAN 从 neobanker + 12 参考平台复用代码 / 架构 / 操作模式。**每一个可见像素都是原创**。任何 LIULIAN 表面截图都不应被误认为：
...
每个 PR 跑 `conventions/UI_AUDIT_CHECKLIST.md §I` 的 4 项测试：AI-slop / category-reflex / neobanker-copy / gui-demo cross-check。任何一项失败阻塞合并。
...
Phosphor Icons (Regular 1.5px stroke) 主，Lucide fallback（neobanker 用，代码复用一致）。领域特定 glyph 自画 24px grid 1.5px stroke。表格 cell 内**禁图标**。
...
3 agent persona；同引擎，不同工具集。SSE 事件形（继承自 neobanker-agent）：
...
Provider 降级 banner（如 Gemini 在 CN 不可用）：`--pastel-blue` 背景 + 同 neobanker `DegradationBanner.tsx` 形状
...
*详细英文权威版：`PLATFORM_DESIGN.md`。架构上游：`PLATFORM_BLUEPRINT.md`。一周执行：`ONE_WEEK_SPRINT.md`。复用计划：`NEOBANKER_REUSE_MAP.md`。每 PR 必过项：`conventions/UI_AUDIT_CHECKLIST.md`。*
