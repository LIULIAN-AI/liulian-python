...
def save_conflict_resolutions(
    artifact_dir: Path, resolutions: List[Dict[str, Any]]
) -> None:
...
def record_change(artifact_dir: Path, change: ChangeRecord) -> None:
    """
...
def record_test_results(artifact_dir: Path, step_id: str, test_output: str) -> None:
...
def record_commit(
    artifact_dir: Path, step_id: str, commit_info: Dict[str, Any]
) -> None:
...
def generate_final_report(artifact_dir: Path, run: AdaptationRun) -> Tuple[str, str]:
    """
    ...
    # Changes summary
    md_lines.extend(
        ['', f'## Changes Summary', '', f'**Total Changes:** {len(run.changes)}', '']
    )
    ...
    if run.conflicts_resolved:
        md_lines.extend(['## Conflicts Resolved', ''])
...
def generate_adaptation_summary(run: AdaptationRun) -> str:
    ...
    total_commits = len(
        [c for c in run.changes if c.change_type in ['create', 'modify']]
    )
    ...
    conflicts_remaining = len(
        [c for c in run.conflicts_resolved if not c.get('resolved', False)]
    )
