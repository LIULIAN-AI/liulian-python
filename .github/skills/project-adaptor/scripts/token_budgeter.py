...
class TokenBudgeter:
    ...
    def track_usage(
        self, step_id: str, prompt_tokens: int, response_tokens: int
    ) -> None:
...
"""
    ...
    def generate_file_summary(self, file_path: Path, max_lines: int = 3) -> str:
        ...
        try:
            ...
            if classes:
                class_names = [
                    c.split('(')[0].replace('class ', '') for c in classes[:3]
                ]
                summary_parts.append(f'Classes: {", ".join(class_names)}')

            if functions:
                func_names = [
                    f.split('(')[0].replace('def ', '') for f in functions[:5]
                ]
    ...
    def generate_diff_only(
        self, original_content: str, modified_content: str, context_lines: int = 3
    ) -> str:
        """
    ...
    def cache_prompt_response(
        self, prompt: str, context_files: List[str], response: str
    ) -> None:
        """
