...
def parse_args() -> argparse.Namespace:
    ...
    g.add_argument('--data', default='swiss-river-1990',
                   choices=['swiss-river-1990', 'swiss-river-2010',
                            'swiss-river-zurich'],
                   help='Dataset variant.')
    g.add_argument('--seq_len', type=int, default=90,
                   help='Look-back window size (0 → full history).')
    g.add_argument('--pred_len', type=int, default=7,
                   help='Forecast horizon (num future steps).')
    g.add_argument('--features', default='M',
    ...
    g.add_argument('--task', default='forecast',
                   choices=['forecast', 'nowcast'],
                   help='Prediction task type.')
    g.add_argument('--split_mode', default='ts',
    ...
    g.add_argument('--graph_mode', default='none',
                   choices=['none', 'edge_index', 'adj_matrix',
                            'graphlet_features'],
                   help='How graph info is exposed to the model.')
    g.add_argument('--graphlet_num_hops', type=int, default=1,
                   help='Neighbourhood radius for graphlet features.')

    # ----- Model architecture -------------------------------------------
    g = p.add_argument_group('Model architecture')
    g.add_argument('--enc_in', type=int, default=None,
    ...
    g.add_argument('--loss', default='mse', choices=['mse', 'mae', 'rmse'],
                   help='Training loss used by ForecastTrainer.')
    g.add_argument('--metrics', default='rmse,mae,nse',
                   help='Comma-separated eval metrics. '
                        'Supported: mse, mae, rmse, nse.')
    g.add_argument('--eval_denorm', action='store_true', default=True,
                   help='Compute metrics in original data scale (after inverse transform).')
    g.add_argument('--no_eval_denorm', action='store_false', dest='eval_denorm',
                   help='Disable denormalized metric computation.')
    g.add_argument('--show_progress', action='store_true', default=True,
                   help='Show tqdm progress bars for train/val/test loops.')
    g.add_argument('--no_progress', action='store_false', dest='show_progress',
                   help='Disable tqdm progress bars.')
    g.add_argument('--patience', type=int, default=10)
    g.add_argument('--lradj', default='none',
                   choices=['none', 'constant', 'type1', 'type2', 'type3',
                            'PEMS', 'COS', 'cosine_warmup', 'onecycle',
                            'step', 'multistep', 'exponential', 'plateau',
                            'TST'],
                   help='LR schedule type. Swiss River LSTM uses "none" '
                        '(constant Adam LR). See liulian.optim.lr_schedulers '
                        'for all options.')
    g.add_argument('--pct_start', type=float, default=0.2,
                   help='OneCycleLR warm-up fraction.')
    g.add_argument('--cos_T_max', type=int, default=20,
                   help='CosineAnnealingLR T_max.')
    g.add_argument('--cos_eta_min', type=float, default=1e-8,
                   help='CosineAnnealingLR / CosineWarmRestarts eta_min.')
    g.add_argument('--cos_T_0', type=int, default=10,
                   help='CosineAnnealingWarmRestarts T_0.')
    g.add_argument('--cos_T_mult', type=int, default=2,
                   help='CosineAnnealingWarmRestarts T_mult.')
    g.add_argument('--step_size', type=int, default=10,
                   help='StepLR step_size (epochs).')
    g.add_argument('--gamma', type=float, default=0.5,
                   help='Decay factor for StepLR/ExponentialLR/MultiStepLR.')
    g.add_argument('--milestones', default='30,60,90',
                   help='Comma-separated epoch milestones for MultiStepLR.')
    g.add_argument('--sched_patience', type=int, default=5,
                   help='ReduceLROnPlateau patience.')
    g.add_argument('--sched_factor', type=float, default=0.5,
    ...
    g.add_argument('--wandb_entity', default=None,
                   help='wandb team/user entity.')
    g.add_argument('--dev_run', action='store_true',
                   help='Dev mode: disables wandb even if project is set.')

    # ----- HPO -----------------------------------------------------------
    g = p.add_argument_group('Hyperparameter optimisation')
    g.add_argument('--hpo', action='store_true', default=True,
                   help='Enable Ray Tune hyperparameter search.')
    g.add_argument('--hpo_num_samples', type=int, default=200,
                   help='Number of HPO trials (Swiss River ref default: 200).')
    g.add_argument('--hpo_scheduler', default='asha',
                   choices=['asha', 'none'],
                   help='Ray Tune scheduler.')
    g.add_argument('--hpo_grace_period', type=int, default=5,
                   help='ASHA min epochs before pruning '
                        '(Swiss River ref: 3 for vanilla, 5 for embedding).')
    g.add_argument('--hpo_reduction_factor', type=float, default=1.5,
                   help='ASHA bracket reduction factor '
                        '(Swiss River ref: 2 for vanilla, 1.5 for embedding).')
    g.add_argument('--hpo_storage_path', type=str, default=None,
                   help='Directory for Ray Tune output '
                        '(default: artifacts/ray_results).')
    g.add_argument('--hpo_resources_cpu', type=int, default=1,
                   help='CPUs per trial (default: 1).')
    g.add_argument('--hpo_resources_gpu', type=float, default=0.25,
                   help='GPUs per trial — use fractional (e.g. 0.25) '
                        'to share one GPU across trials (default: 0).')
    g.add_argument('--hpo_num_cpus', type=int, default=None,
                   help='Total CPUs for ray.init(). '
                        'Negative = cpu_count - N. None = auto.')
    g.add_argument('--hpo_max_concurrent', type=int, default=None,
                   help='Max concurrent trials (default: unlimited).')
    g.add_argument('--hpo_resume', action='store_true', default=False,
                   help='Resume a previously interrupted HPO run.')
    g.add_argument('--hpo_save_checkpoints', action='store_true', default=True,
                   help='Save .pth checkpoints per trial (default: True).')
    g.add_argument('--hpo_no_save_checkpoints', action='store_false',
                   dest='hpo_save_checkpoints',
                   help='Disable checkpoint saving.')
    g.add_argument('--hpo_trim_checkpoints', action='store_true', default=True,
                   help='Trim .pth files after HPO to save disk (default: True).')
    g.add_argument('--hpo_no_trim', action='store_false',
                   dest='hpo_trim_checkpoints',
                   help='Disable post-HPO checkpoint trimming.')
    g.add_argument('--hpo_keep_best_n', type=int, default=10,
                   help='Number of best trials to keep when trimming '
                        '(default: 10).')
    g.add_argument('--hpo_trim_best_n', action='store_true', default=True,
                   help='Also trim checkpoints within the best-N trials '
                        '(default: True).')
    g.add_argument('--hpo_no_trim_best_n', action='store_false',
                   dest='hpo_trim_best_n',
                   help='Keep all checkpoints for the best-N trials.')
    g.add_argument('--hpo_trim_keep_best', action='store_true', default=True,
                   help='Keep best-epoch checkpoint for trimmed trials '
                        '(default: True).')
    g.add_argument('--hpo_no_trim_keep_best', action='store_false',
                   dest='hpo_trim_keep_best',
                   help='Remove all checkpoints for trimmed trials.')
    g.add_argument('--hpo_trim_keep_last', action='store_true', default=False,
                   help='Keep last-epoch checkpoint for trimmed trials '
                        '(default: False).')
    g.add_argument('--hpo_experiment_name', type=str, default=None,
    ...
    g.add_argument('--hpo_local_mode', action='store_true', default=False,
    ...
    g.add_argument('--auto_viz', action='store_true', default=True,
                   help='Auto-generate plots after training.')
    g.add_argument('--no_viz', action='store_false', dest='auto_viz',
                   help='Disable auto-visualisation.')
    g.add_argument('--viz_method', default='mean',
...
def _print_dataset_summary(dataset: SwissRiverDataset) -> None:
    ...
    info = dataset.info()
    logger.info('Dataset: %s  (mode=%s, graph=%s)',
                info.get('data_name'), info.get('split_mode'),
                info.get('graph_name', 'none'))
    logger.info('  stations=%d, task=%s, seq_len=%d, pred_len=%d',
                info.get('num_stations', 0), info.get('task', ''),
                info.get('seq_len', 0), info.get('pred_len', 0))
    logger.info('  noise=%s, identifier=%s [%s], gap=%s',
    ...
    for name in ('train', 'val', 'test'):
        ...
        logger.info('  split %-5s  samples=%d  feat=%d  targ=%d',
                     name, len(split), split.feat_dim, split.targ_dim)
...
def main() -> None:
    ...
    if config['identifier_mode'] == 'embedding':
        ...
        logger.info('EntityWrapper: num_embeddings=%d, embedding_size=%d',
                    num_emb, config['embedding_size'])
    ...
    if config.get('hpo'):
        ...
        optimizer = RayOptimizer(config={
            ...
            'experiment_name': _build_hpo_experiment_name(
                config, args.model),
            'local_mode': config.get('hpo_local_mode', False),
        })
        # Default search space for LSTM / TimeLLM
        config['search_space'] = config.get('search_space', {
            ...
            'learning_rate': ray.tune.uniform(0.00001, 0.01),  # [0.0001, 0.0005, 0.001, 0.005],  # 10x less
            # 'dropout': [0.05, 0.1, 0.2],
        })
        logger.info('HPO enabled: %d samples, grace_period=%s, reduction_factor=%s, '
    ...
    print(f'Task       : {config["task"]}  (seq={config["seq_len"]}, '
